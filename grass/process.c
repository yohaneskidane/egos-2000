...
void proc_init() {
    ...
    asm("csrw pmpcfg0, %0" ::"r"(0x1B111D0F));
    // 0x0F
    ...
    asm("csrw pmpaddr1, %0" ::"r"((0x20400000 >> 2) | 0b01111111111111111111));

    /* Setup PMP NAPOT region 0x20800000 - 0x20C00000 as r/-/- */
    // asm("csrw pmp2cfg, %0" ::"r"(0x11));
    asm("csrw pmpaddr2, %0" ::"r"((0x20800000 >> 2) | 0b01111111111111111111));

    /* Setup PMP NAPOT region 0x80000000 - 0x80004000 as r/w/- */
    // asm("csrw pmp3cfg, %0" ::"r"(0x1B));
    asm("csrw pmpaddr3, %0" ::"r"((0x80000000 >> 2) | 0b011111111111));//0xFFF));
    /* Student's code ends here. */
